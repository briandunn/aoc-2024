...
let tryItem (x, y) grid =
    if Array2D.length1 grid > x && Array2D.length2 grid > y && x >= 0 && y >= 0 then
...
let fromSeq (rows: 'a seq seq) : 'a Grid =
    ...
    rows
    |> Seq.iteri (fun i row -> row |> Seq.iteri (fun j cell -> grid.[i, j] <- Some cell))

    grid |> Array2D.map Option.get

let fromLines (map: char -> 'a) (lines: string seq) : 'a Grid =
    let rows = lines |> Seq.map (fun line -> line.ToCharArray()) |> Seq.toArray
